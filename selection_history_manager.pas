...
begin
  ...
  begin
    ...
    SelectionHistory.AddOrSetData(keyBuf, (t as TDbMemo).SelText);
    // upsert selected
    // Cutting
...
begin
  ...
  begin
    ...
    (t as TDBMemo).Lines[Line] :=
      Copy((t as TDBMemo).Lines[Line], 1, CharIndex) + dialog.InsertedText +
      Copy((t as TDBMemo).Lines[Line], CharIndex + 1, Length(
      (t as TDBMemo).Lines[Line]) - CharIndex);
